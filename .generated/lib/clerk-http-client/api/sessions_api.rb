...
module ClerkHttpClient
  class SessionsApi
    ...
    def refresh_session(session_id, opts = {})
      data, _status_code, _headers = refresh_session_with_http_info(session_id, opts)
      data
    end
    ...
    # POST 
    def refresh_session_with_http_info(session_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: SessionsApi.refresh_session ...'
      end
      # verify the required parameter 'session_id' is set
      if @api_client.config.client_side_validation && session_id.nil?
        fail ArgumentError, "Missing the required parameter 'session_id' when calling SessionsApi.refresh_session"
      end
      # resource path
      local_var_path = '/sessions/{session_id}/refresh'.sub('{' + 'session_id' + '}', CGI.escape(session_id.to_s))
      ...
      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(opts[:'refresh_session_request'])

      # return_type
      return_type = opts[:debug_return_type] || 'SessionRefresh'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['bearerAuth']

      new_options = opts.merge(
        :operation => :"SessionsApi.refresh_session",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      data, status_code, headers = @api_client.call_api(:POST, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: SessionsApi#refresh_session\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
