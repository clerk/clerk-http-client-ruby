  ...
  class CreateOAuthApplicationRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClerkHttpClient::CreateOAuthApplicationRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClerkHttpClient::CreateOAuthApplicationRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'consent_screen_enabled')
        ...
        self.consent_screen_enabled = true
      end
    ...
    def list_invalid_properties
      ...
      if !@callback_url.nil? && @callback_url.to_s.length > 1024
        invalid_properties.push('invalid value for "callback_url", the character length must be smaller than or equal to 1024.')
      end

      if !@scopes.nil? && @scopes.to_s.length > 1024
        invalid_properties.push('invalid value for "scopes", the character length must be smaller than or equal to 1024.')
      end
    ...
    def valid?
      ...
      return false if !@callback_url.nil? && @callback_url.to_s.length > 1024
      return false if !@scopes.nil? && @scopes.to_s.length > 1024
      true
    end
    ...
    def callback_url=(callback_url)
      if !callback_url.nil? && callback_url.to_s.length > 1024
        fail ArgumentError, 'invalid value for "callback_url", the character length must be smaller than or equal to 1024.'
      end
    ...
    def scopes=(scopes)
      if !scopes.nil? && scopes.to_s.length > 1024
        fail ArgumentError, 'invalid value for "scopes", the character length must be smaller than or equal to 1024.'
      end
    ...
    def hash
      [name, redirect_uris, callback_url, scopes, consent_screen_enabled, pkce_required, public].hash
    end
