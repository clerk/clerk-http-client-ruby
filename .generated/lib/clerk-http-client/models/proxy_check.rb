  ...
  class ProxyCheck
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClerkHttpClient::ProxyCheck` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClerkHttpClient::ProxyCheck`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      object_validator = EnumAttributeValidator.new('String', ["proxy_check"])
    ...
    def object=(object)
      validator = EnumAttributeValidator.new('String', ["proxy_check"])
    ...
    def hash
      [object, id, domain_id, last_run_at, proxy_url, successful, created_at, updated_at].hash
    end
