  ...
  class UpdateInstanceAuthConfig200Response
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClerkHttpClient::UpdateInstanceAuthConfig200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClerkHttpClient::UpdateInstanceAuthConfig200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      object_validator = EnumAttributeValidator.new('String', ["instance_settings"])
    ...
    def object=(object)
      validator = EnumAttributeValidator.new('String', ["instance_settings"])
    ...
    def hash
      [object, id, restricted_to_allowlist, from_email_address, progressive_sign_up, enhanced_email_deliverability].hash
    end
