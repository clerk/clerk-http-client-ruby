  ...
  class UpdateOrganizationRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClerkHttpClient::UpdateOrganizationRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClerkHttpClient::UpdateOrganizationRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@max_allowed_memberships.nil? && @max_allowed_memberships < 0
        invalid_properties.push('invalid value for "max_allowed_memberships", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@max_allowed_memberships.nil? && @max_allowed_memberships < 0
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] max_allowed_memberships Value to be assigned
    def max_allowed_memberships=(max_allowed_memberships)
      if !max_allowed_memberships.nil? && max_allowed_memberships < 0
        fail ArgumentError, 'invalid value for "max_allowed_memberships", must be greater than or equal to 0.'
      end
    ...
    def hash
      [public_metadata, private_metadata, name, slug, max_allowed_memberships, admin_delete_enabled, created_at].hash
    end
