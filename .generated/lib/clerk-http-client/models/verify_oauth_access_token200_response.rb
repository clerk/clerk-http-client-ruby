  ...
  class VerifyOAuthAccessToken200Response
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClerkHttpClient::VerifyOAuthAccessToken200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClerkHttpClient::VerifyOAuthAccessToken200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^oat_[0-9A-Fa-f]{32}$/)
      if @id !~ pattern
    ...
    def valid?
      ...
      object_validator = EnumAttributeValidator.new('String', ["clerk_idp_oauth_access_token"])
      return false unless object_validator.valid?(@object)
      return false if @id.nil?
      return false if @id !~ Regexp.new(/^oat_[0-9A-Fa-f]{32}$/)
      return false if @client_id.nil?
      return false if @subject.nil?
      return false if @subject !~ Regexp.new(/^user_\w{27}$/)
    ...
    def object=(object)
      validator = EnumAttributeValidator.new('String', ["clerk_idp_oauth_access_token"])
    ...
    def hash
      [object, id, client_id, subject, scopes, revoked, revocation_reason, expired, expiration, created_at, updated_at].hash
    end
