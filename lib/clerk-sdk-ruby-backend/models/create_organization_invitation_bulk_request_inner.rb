...
module ClerkBackend
  class CreateOrganizationInvitationBulkRequestInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClerkBackend::CreateOrganizationInvitationBulkRequestInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClerkBackend::CreateOrganizationInvitationBulkRequestInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @inviter_user_id.nil?
        invalid_properties.push('invalid value for "inviter_user_id", inviter_user_id cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @inviter_user_id.nil?
    ...
    def hash
      [email_address, inviter_user_id, role, public_metadata, private_metadata, redirect_url].hash
    end
